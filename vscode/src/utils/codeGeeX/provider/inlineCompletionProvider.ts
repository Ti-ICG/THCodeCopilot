...
let lastResultItems = [];
let lastPosition: vscode.Position = null;
let trie = new Trie([]);
let prompts: string[] = [];
...
function removeTrailingCharsByReplacement(completion: string, replacement: string) {
...
export default function inlineCompletionProvider(
  ...
  const provider: vscode.InlineCompletionItemProvider = {
    provideInlineCompletionItems: async (document, position, context, token) => {
      ...
      const enableExtension = await extensionContext.globalState.get('EnableExtension', true);
      ...
      if (true && !reGetCompletions) {
        for (let prompt of prompts) {
          if (textBeforeCursor.trimEnd().indexOf(prompt) != -1) {
            ...
            for (
              let i = 0;
              i < Math.min(Math.min(completions.length, candidateNum) + 1, completions.length);
              i++
            ) {
              ...
              let needRequest = ['', '\n', '\n\n'];
              if (needRequest.includes(insertText) || insertText.trim() === '') {
                continue;
              }
              if (useTrim) {
      ...
      if (enableExtension && textBeforeCursor.length > 8) {
        if (lastPosition?.line == position.line && lastPosition?.character == position.character) {
          // 这里做了消抖处理
          return lastResultItems;
        }
        ...
        try {
          ...
          //   'inlinecompletion'
          // );
          let textAfterCursor = editor.document.getText(
            new vscode.Range(position.line, position.character, editor.document.lineCount, 0)
          );

          // rs = await CodeAssistant.codeCompletionByFIM(textBeforeCursor, textAfterCursor);
          rs = await CodeAssistant.codeCompletion(textBeforeCursor);
        ...
        for (let i = 0; i < rs.completions.length; i++) {
          ...
          if (isAtTheMiddleOfLine(editor, document)) {
            ...
            if (
              ...
              completion = completion.trimEnd().substring(0, completion.length - 1);
            }
          }
          items.push({
        ...
        for (let j = 0; j < items.length; j++) {
          items[j].command = {
            ...
            arguments: [rs.commandid, rs.completions, items[j].insertText],
          };
        }
